...
async function resolveFile(texturePath, mtlPath) {
...
export async function loadTexture(texturePath, mtlPath, options) {
  options = options ?? {};
  ...
  const { source, name, extension } = await resolveFile(texturePath, mtlPath);
