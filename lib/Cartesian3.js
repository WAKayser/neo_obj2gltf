 ...
 * @param {number[]} [result] The array onto which to store the result. If this is a typed array, it must have array.length * 3 components, else a {@link Error} will be thrown. If it is a regular array, it will be resized to have (array.length * 3) elements.
...
Cartesian3.clamp = function (value, min, max, result) {
  ...
  const z = clamp(value.z, min.z, max.z);
